...
//  (with sentence-aligned corpus, without word alignments)
//
...
int get_cooccurrence() {
    int flag, x, y, fidcounter = 1, sent1_len = 0, sent2_len = 0, read_mod = 0;
    long long a, j = 0, k, id, counter = 0, ind = 0, vocab_size1, vocab_size2, w1, w2, *lookup, *sentence1, *sentence2;
	...
	if(flag == 1) {		// Newline
		...
		for (j = 0; j < sent1_len; ++j) {
		    for (k = 0; k < sent2_len; ++k) {
			w1 = sentence1[j];
			w2 = sentence2[k];
	...
	if (read_mod == 0) {
	    ...
	    sentence1[sent1_len++] = htmp->id;
	} else if (read_mod == 1) {
	    htmp = hashsearch(vocab_hash2, str);
	    if (htmp == NULL) continue;
	    sentence2[sent2_len++] = htmp->id;
	}
    }
    }
